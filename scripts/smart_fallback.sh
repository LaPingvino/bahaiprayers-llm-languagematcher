...
# Check Gemini CLI (uses authentication, not API key)
if command -v gemini &> /dev/null; then
    GEMINI_AVAILABLE=true
    echo "  ✅ Gemini CLI: Available (authenticated)" | tee -a "$LOG_FILE"
else
...
if command -v ollama &> /dev/null; then
    ...
    echo "  ✅ ollama: Available (local fallback)" | tee -a "$LOG_FILE"
else
    echo "  ❌ ollama: Not found" | tee -a "$LOG_FILE"
fi
...
if ! $CLAUDE_AVAILABLE && ! $GEMINI_AVAILABLE && ! $GPTOSS_AVAILABLE; then
    ...
    echo "  - ollama: https://ollama.com/" | tee -a "$LOG_FILE"
    exit 1
fi
...
echo "Strategy: Claude → Gemini → ollama (local)" | tee -a "$LOG_FILE"
echo "" | tee -a "$LOG_FILE"
...
# Try ollama as last resort (local, always works but slow)
if $GPTOSS_AVAILABLE && ! $SUCCESS; then
    echo "" | tee -a "$LOG_FILE"
    echo "Phase 3: Trying ollama (local fallback)" | tee -a "$LOG_FILE"
    echo "⏳ Note: ollama is slower but has no rate limits" | tee -a "$LOG_FILE"

    if try_backend "ollama" "-gpt-oss" "ollama (local)"; then
        SUCCESS=true
        FINAL_BACKEND="ollama (local)"
    else
        echo "  ollama failed, trying to process saved batches..." | tee -a "$LOG_FILE"

        # Try to process any saved batches with ollama
        echo "  🔄 Processing all saved batches with ollama..." | tee -a "$LOG_FILE"
        process_saved_batches "-gpt-oss" "ollama (local)"
    fi
fi
...
echo "  ollama:     $($GPTOSS_AVAILABLE && echo "✅ Available" || echo "❌ Missing")" | tee -a "$LOG_FILE"
